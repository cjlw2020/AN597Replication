...
```{r}
ggplot(data = df, aes(age,mass)) +
  geom_smooth(model = model.logistic, color = "red", size = 0.5)  +
  geom_point(aes(age,mass)) +
  theme_test()
...
```{r}
#Lord Model 
L <- function(age, theta1, theta2, theta3){t1(exp(-(t3)/age + 0.62))}
