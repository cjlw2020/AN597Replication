...
t <- ggplot(data = df, aes(age,mass)) +
  geom_smooth(model = model.logistic, color = "red", size = 0.5, linetype = 2, alpha = 0.15)  +
...
#Lord Model: I dont know why they don't work
```{r}
#Lord Model 
L <- function(age,theta1,theta2,theta3){theta1*(exp(-theta3/age + 0.62))}
  
model.lord <- nls(theta1*(exp(-theta3/age + 0.62)), data=df, start=list(theta1=t1,theta2=t2,theta3=t3))
```
or...
```{r}
L <- function(age,theta1,theta2,theta3){theta1*(exp(-(theta3)/age + 0.62))}

model.lord <- nls(mass ~ L(age,theta1,theta2,theta3),data=df, start=list(theta1=t1,theta2=t2,theta3=t3))
